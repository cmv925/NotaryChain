...
|---------|---------|------|
| **Smart Contract Template Library** (NEW): browse 17 curated real-world legal-agreement templates (6 categories) → fill structured fields → optional GPT-5.2 clause tailoring (graceful non-AI fallback) → **anchor the finalized agreement on Hedera HCS** (SHA-256 hash → `hedera_service.seal_document` → tx id + HashScan explorer URL; identity-gated, reuses notarization gate + EnhancedKBA). Backend: `legal_templates.py` + `routes/contract_template_routes.py` (`GET /api/contract-templates`, `/detail/{id}`, `POST /render/{id}` (ai_tailor), `POST /anchor`, `GET /anchors/my`, `/anchors/{id}`). Frontend: `SmartContractTemplates.jsx` + `AnchorOnChainModal.jsx` + `SmartContractsPage.jsx` (route `/smart-contracts`, open to all logged-in users), a "Smart Contract Templates" tab in the AI Doc Generator, and an "Anchor on Blockchain" CTA on the Template Library. | iter121: backend 11/11 pytest + frontend 100% (library/search/filter/editor/render/AI/anchor-gate/KBA/CTA/tab). Test: `backend/tests/test_smart_contract_templates.py` | May 31, 2026 |
