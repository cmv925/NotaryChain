...
|---------|---------|------|
| **Oversized-component refactor (tech debt, P2)**: Split the three largest React components into focused, maintainable units with no behavior change. (1) `OperationsTab.jsx` 683→85 lines — extracted 8 panels into `admin/tabs/operations/` (SystemStatusStrip, HederaSection, StoragePaymentsSection, BalanceAlertsSection, ServiceHealthPanel, IncidentsPanel, StorageAnalyticsPanel, AlertSettingsPanel). (2) `ANANDashboard.jsx` 877→280 lines — extracted `anan/AnanListView.jsx` + `anan/AnanDetailView.jsx`, moved AgentScoreCard/StatCard/InfoRow into `anan/AnanPrimitives.jsx`, added `handleVerifyBond`/`copyToClipboard` helpers. (3) `BiometricVerification.jsx` 617→228 lines — all camera/TensorFlow/liveness/challenge logic moved into `hooks/useBiometricVerification.js` (364 lines), component is now presentational. | Lint clean (all files), webpack compiled successfully; ANAN list view + admin Operations tab visually verified rendering all sub-components. Biometric: lint + compile verified (logic moved verbatim; not camera E2E re-tested). | Feb 2026 |
