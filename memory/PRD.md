...
|---------|---------|------|
| **Ceremony Video Vault (P1)**: hybrid tamper-evident RON recording storage — AWS S3 stores the bytes, Hedera anchors only the SHA-256. Backend `routes/ceremony_video_routes.py`: chunked S3 **multipart** upload (`/init`, `/{id}/part`, `/{id}/complete`, `/{id}/abort`) where the browser splits the file into 8MB parts (bypasses proxy limit); a detached `asyncio.create_task` finalize streams the assembled object back from S3 for an **authoritative server-side SHA-256** (never trusts the client), anchors it via `hedera_service.seal_document`, then applies S3 Object Lock (WORM) retention gracefully. Plus `/{id}` (RBAC), `/mine`, `/{id}/playback` (short-lived presigned URL), `/{id}/verify` (re-hash tamper check), and PUBLIC `/verify/{hash}` (no auth, never returns video). `StorageService` extended with multipart/compute_sha256/apply_object_lock/head_size (sync, run via `asyncio.to_thread`). SSE-S3 default (SSE-KMS if `AWS_KMS_KEY_ID` set). Frontend: `CeremonyVideoVault.jsx` (chunked upload + Web Crypto client hash + progress bar + anchor polling + recordings list + detail modal w/ verify link & secure playback), `CeremonyVaultPage.jsx` (`/ceremony-vault`), public `VerifyCeremonyVideo.jsx` (`/verify/recording/:hash`). | Verified e2e: real S3 multipart upload (curl + browser UI) → server re-hash matched client hash → Hedera anchor → public "Verified & Untampered" page. All lint clean, both services compiled. NOTE: Object Lock returned false (bucket `notarychain-documents` is not Object-Lock-enabled — graceful fallback worked; true WORM needs an Object-Lock-enabled bucket). HCS submission is local-seal in preview (no default topic). | Feb 2026 |
| **Oversized-component refactor (tech debt, P2)**: Split the three largest React components into focused, maintainable units with no behavior change. (1) `OperationsTab.jsx` 683→85 lines — extracted 8 panels into `admin/tabs/operations/`. (2) `ANANDashboard.jsx` 877→280 lines — extracted `anan/AnanListView.jsx` + `anan/AnanDetailView.jsx`, moved AgentScoreCard/StatCard/InfoRow into `anan/AnanPrimitives.jsx`. (3) `BiometricVerification.jsx` 617→228 lines — all camera/TensorFlow/liveness/challenge logic moved into `hooks/useBiometricVerification.js`. | Lint clean, webpack compiled; ANAN list view + admin Operations tab visually verified. | Feb 2026 |
