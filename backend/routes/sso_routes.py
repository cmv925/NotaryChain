...
Handles org-based SSO discovery, initiation, and simulated SAML/OIDC callback flows.
...
def set_db(database):
    global db
    db = database
    # Share DB with common module
    from routes import sso_common
    sso_common.set_db(database)
...
async def sso_callback(body: SSOCallbackRequest):
    ...
    if not user:
        ...
        logger.info("SSO JIT provisioned user: %s for org: %s", email, org_id)
    else:
...
async def test_sso_config(body: SSOTestRequest, current_user: dict = Depends(get_current_user)):
    ...
    if issues:
        return {"success": False, "message": "SSO configuration has issues", "issues": issues}
...
async def sso_providers():
    ...
    okta_client = os.environ.get("OKTA_CLIENT_ID", "")

    if auth0_domain and auth0_client:
        providers.append({"name": "Auth0", "type": "auth0", "configured": True})
    if okta_domain and okta_client:
...
async def cleanup_sso_sessions():
    ...
    if result.deleted_count > 0:
        logger.info("Cleaned up %d expired SSO sessions", result.deleted_count)
