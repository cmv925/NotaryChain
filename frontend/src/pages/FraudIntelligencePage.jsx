...
const SEVERITY_MAP = {
  ...
  high: { cls: 'bg-coral-500/20 text-coral-600 border-coral-200', label: 'HIGH' },
  medium: { cls: 'bg-coral-500/20 text-coral-600 border-gold-500/30', label: 'MEDIUM' },
  low: { cls: 'bg-slate-500/20 text-slate-600 border-slate-500/30', label: 'LOW' },
};
    ...
    <div className="min-h-screen bg-navy-900 text-navy-900">
      <div className="bg-cream-100 border-b border-slate-200 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-navy-900 font-bold text-lg tracking-tight">Fraud Intelligence</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="flex gap-1 mb-6 bg-cream-100 rounded-lg p-1 w-fit" data-testid="fraud-tabs">
          ...
          <button onClick={() => setTab('ron')} className={`px-4 py-2 text-xs font-bold rounded ${tab === 'ron' ? 'bg-coral-500/20 text-coral-600' : 'text-slate-500 hover:text-navy-900'}`}>
                  ...
                  <Card key={p.pattern_id} className={`bg-cream-100 border-slate-200 ${!p.active ? 'opacity-50' : ''}`}
                    ...
                    <CardContent className="p-4 flex items-start justify-between gap-4">
                      <div className="flex items-start gap-3 flex-1">
                        <div className="w-8 h-8 rounded bg-cream-200 flex items-center justify-center mt-0.5">
                          ...
                          {p.indicators?.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                              {p.indicators.map(ind => (
                                <span key={ind} className="px-1.5 py-0.5 text-[9px] bg-cream-200 text-slate-600 rounded">{ind}</span>
                              ))}
                            </div>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => handleToggle(p.pattern_id)} title={p.active ? 'Deactivate' : 'Activate'}
                          className="p-1.5 rounded hover:bg-cream-200 transition-colors" data-testid={`fraud-toggle-${p.pattern_id}`}>
                          {p.active ? <ToggleRight className="w-5 h-5 text-coral-600" /> : <ToggleLeft className="w-5 h-5 text-slate-600" />}
                        </button>
              ...
              <Card key={r.jurisdiction} className="bg-cream-100 border-slate-200"
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpandedRule(expandedRule === r.jurisdiction ? null : r.jurisdiction)}>
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded flex items-center justify-center ${r.ron_enabled ? 'bg-coral-500/10' : 'bg-red-500/10'}`}>
                        <Building className={`w-4 h-4 ${r.ron_enabled ? 'text-coral-600' : 'text-red-400'}`} />
                      </div>
                      <div>
                        <h4 className="text-navy-900 font-semibold text-sm">{r.state_name}
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`px-2 py-1 text-[9px] font-bold border rounded ${r.ron_enabled ? 'bg-coral-500/20 text-coral-600 border-coral-200' : 'bg-red-500/20 text-red-400 border-red-500/30'}`}>
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-200 grid grid-cols-2 gap-2 text-[11px]">
                      {r.requirements && Object.entries(r.requirements).map(([key, val]) => (
                        <div key={key} className="flex items-center justify-between">
                          ...
                          <span className={`font-medium ${val === true ? 'text-coral-600' : val === false ? 'text-red-400' : 'text-navy-900'}`}>
                      ...
                      {r.notes && <p className="col-span-2 text-slate-600 text-[10px] mt-1">{r.notes}</p>}
                    </div>
                  )}
                </CardContent>
              </Card>
