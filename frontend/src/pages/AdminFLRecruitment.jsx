    ...
    <Shell>
      <div className="max-w-7xl mx-auto" data-testid="admin-fl-recruitment-page">
        <div className="flex items-start justify-between gap-3 mb-6 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-emerald-400 text-[10px] uppercase tracking-[0.25em] font-bold">Florida · Recruitment Pipeline</span>
            </div>
            <h1 className="text-3xl font-bold">FL notary leads</h1>
            <p className="text-slate-400 text-sm mt-1">Track every prospective Florida notary from interest to onboarded.</p>
          </div>
          <Button onClick={load} variant="outline" className="bg-slate-900/60 border-slate-700 text-white hover:bg-slate-800" data-testid="refresh-leads-btn">
        ...
        <div className="mb-3 text-xs text-slate-400">
          {statusFilter ? <>Filtered by <span className="text-emerald-300 font-bold">{statusFilter}</span> · <button onClick={() => setStatusFilter('')} className="text-emerald-400 hover:underline">clear</button></> : 'All leads'}
          {stats && <> · {stats.conversion_rate}% conversion · {stats.last_30d} in 30d</>}
        </div>

        {/* Leads list */}
        <Card className="bg-slate-900/60 border-slate-800" data-testid="leads-list">
          <CardContent className="p-0">
            ...
            {leads.map(lead => (
              <button key={lead.lead_id} onClick={() => setSelected(lead)} className="w-full px-5 py-3 border-b border-slate-800/60 hover:bg-slate-800/30 grid grid-cols-12 gap-2 text-xs items-center text-left" data-testid={`lead-row-${lead.lead_id}`}>
                <span className="col-span-3 text-white font-medium truncate">{lead.full_name}</span>
                <span className="col-span-3 text-slate-400 truncate">{lead.email}</span>
                ...
                <span className="col-span-1 text-right">
                  <span className={`inline-block px-2 py-0.5 rounded text-[10px] uppercase tracking-wider font-bold ${STATUS_STYLES[lead.status] || 'bg-slate-700 text-slate-300'}`}>{lead.status}</span>
                </span>
              </button>
            ))}
          </CardContent>
...
function StatCard({ label, value, onClick, active, testId }) {
  return (
    <button onClick={onClick} disabled={!onClick} className={`text-left bg-slate-900/60 border rounded-lg p-3 transition-colors ${active ? 'border-emerald-500' : 'border-slate-800'} ${onClick ? 'hover:border-slate-600 cursor-pointer' : ''}`} data-testid={testId}>
...
function LeadDetailModal({ lead, token, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4 overflow-auto" onClick={onClose} data-testid="lead-detail-modal">
      <Card className="bg-slate-900 border-slate-800 max-w-2xl w-full my-8" onClick={e => e.stopPropagation()}>
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs bg-slate-950/40 p-3 rounded mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <select value={status} onChange={e => setStatus(e.target.value)} className="bg-slate-950/60 border border-slate-800 rounded-md px-3 h-10 text-sm text-white w-full" data-testid="lead-status-select">
                {STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[10px] uppercase tracking-wider text-slate-500 font-bold block mb-1">Assigned to</label>
              <input value={assignee} onChange={e => setAssignee(e.target.value)} className="bg-slate-950/60 border border-slate-800 rounded-md px-3 h-10 text-sm text-white w-full" placeholder="recruiter@notarychain.com" data-testid="lead-assignee-input" />
            </div>
          </div>
          <div className="mb-3">
            <label className="text-[10px] uppercase tracking-wider text-slate-500 font-bold block mb-1">Internal notes</label>
            <textarea rows={3} value={notes} onChange={e => setNotes(e.target.value)} className="bg-slate-950/60 border border-slate-800 rounded-md p-3 text-sm text-white w-full" data-testid="lead-notes-input" />
          </div>
          <div className="flex justify-end gap-2 mb-4">
            ...
            <Button onClick={save} disabled={saving} className="bg-emerald-600 hover:bg-emerald-500" data-testid="save-lead-btn">
