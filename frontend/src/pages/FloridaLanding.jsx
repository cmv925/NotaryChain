...
export default function FloridaLanding() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white" data-testid="florida-landing">
      {/* Hero */}
      <div className="border-b border-slate-800 bg-gradient-to-b from-orange-950/20 via-slate-950 to-transparent">
        <div className="max-w-6xl mx-auto px-6 py-16 sm:py-24">
          <div className="flex items-center gap-2 mb-4">
            <Sun className="w-5 h-5 text-orange-400" />
            <span className="text-orange-400 text-[11px] uppercase tracking-[0.25em] font-bold">Florida · Online Notary Service</span>
          </div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-5 leading-tight">
            Notarize anything in Florida —<br/>
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-orange-400 via-amber-400 to-emerald-400">
      ...
      <div className="border-b border-slate-800">
        <div className="max-w-6xl mx-auto px-6 py-16">
          ...
          <p className="text-slate-400 max-w-2xl mb-10 text-sm">Florida is one of only a few states allowing online wills. Most legal acts are RON-eligible.</p>
      ...
      <div className="border-b border-slate-800 bg-slate-900/30">
        <div className="max-w-6xl mx-auto px-6 py-16">
          ...
          <p className="text-slate-400 max-w-2xl mb-10 text-sm">Every Florida notarization on NotaryChain enforces the FL DoS requirements automatically.</p>
        ...
        <div className="max-w-6xl mx-auto px-6 py-16">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-slate-400 text-sm">Real notaries, real commissions, every one verified.</p>
            </div>
            <Link to="/notaries?state=FL" className="text-emerald-400 text-sm hover:underline inline-flex items-center gap-1" data-testid="fl-directory-link">
              See all <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          {loading ? (
            <div className="text-center py-10"><Loader2 className="w-8 h-8 animate-spin text-emerald-400 mx-auto" /></div>
          ) : notaries.notaries.length === 0 ? (
            <Card className="bg-slate-900/40 border-dashed border-slate-700" data-testid="fl-no-notaries">
              <CardContent className="p-10 text-center">
                ...
                <Link to="/notary/onboard/florida">
                  <Button className="bg-emerald-600 hover:bg-emerald-500">Onboard now</Button>
                </Link>
              </CardContent>
            </Card>
          ) : (
                  ...
                  <Card className="bg-slate-900/60 border-slate-800 hover:border-emerald-500/40 transition-colors">
                    <CardContent className="p-5">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h3 className="font-bold text-white truncate group-hover:text-emerald-300">{n.name || '—'}</h3>
                      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="bg-emerald-500/5 border-emerald-500/30">
          <CardContent className="p-6">
            <Sun className="w-6 h-6 text-emerald-400 mb-2" />
            <h3 className="text-xl font-bold mb-2">Need something notarized?</h3>
            <p className="text-sm text-slate-400 mb-4">Match with a FL notary, complete the ceremony in 15 minutes.</p>
            <Link to="/request-notarization"><Button className="bg-emerald-600 hover:bg-emerald-500">Start notarization</Button></Link>
          </CardContent>
        </Card>
        <Card className="bg-orange-500/5 border-orange-500/30">
          <CardContent className="p-6">
            ...
            <p className="text-sm text-slate-400 mb-4">Onboard in 6 steps. Get found by clients automatically.</p>
            <Link to="/notary/onboard/florida"><Button variant="outline" className="bg-slate-900/60 border-slate-700 text-white hover:bg-slate-800">Onboard now <ExternalLink className="w-3 h-3 ml-1.5" /></Button></Link>
          </CardContent>
        </Card>
      </div>
    </div>
...
function Stat({ label, value, sub, accent }) {
  const accentClass = accent === 'emerald' ? 'text-emerald-400' : accent === 'amber' ? 'text-amber-400' : 'text-white';
  return (
...
function UseCase({ icon: Icon, title, items, highlight }) {
  return (
    <Card className={`bg-slate-900/40 border-slate-800 ${highlight ? 'ring-1 ring-orange-500/30' : ''}`}>
      <CardContent className="p-6">
        <Icon className={`w-6 h-6 mb-3 ${highlight ? 'text-orange-400' : 'text-emerald-400'}`} />
