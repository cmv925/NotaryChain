...
const RISK_STYLES = {
  low: { c: 'text-coral-700', bg: 'bg-coral-500/10 border-coral-200', icon: ShieldCheck, label: 'Low risk' },
  medium: { c: 'text-coral-700', bg: 'bg-coral-500/10 border-gold-500/30', icon: ShieldAlert, label: 'Medium risk' },
...
export default function FieldScanner() {
  ...
  if (!isAuthenticated) {
    return <Shell><Card className="bg-white border-slate-200 max-w-md mx-auto"><CardContent className="p-8 text-center"><p>Sign in to scan documents.</p><Link to="/login"><Button className="mt-3 bg-coral-500 hover:bg-coral-500">Sign in</Button></Link></CardContent></Card></Shell>;
  }
  ...
  if (result) {
    ...
    const risk = aiSkipped ? { c: 'text-navy-800', bg: 'bg-cream-200/40 border-slate-300', icon: FileImage, label: 'AI check skipped' } : RISK_STYLES[a?.overall_risk || 'medium'];
      ...
      <Shell>
        <div className="max-w-3xl mx-auto" data-testid="scanner-result">
          ...
          <Card className={`mb-4 ${risk.bg}`} data-testid="scan-risk-card">
            <CardContent className="p-5 flex items-center gap-4 flex-wrap">
              ...
              <div className="flex-1 min-w-0">
                ...
                {!aiSkipped && a?.document_summary && <p className="text-xs text-slate-600 mt-2">{a.document_summary}</p>}
                {!aiSkipped && a && !a.ai_powered && <p className="text-[10px] text-coral-600 mt-2">AI degraded fallback — manual review recommended</p>}
          ...
          {result.prior_seal?.found && (
            <Card className="bg-coral-500/5 border-coral-200 mb-4" data-testid="prior-seal-card">
              <CardContent className="p-4 text-sm">
                <p className="font-bold text-coral-700 flex items-center gap-2"><CheckCircle className="w-4 h-4" /> Already sealed on Hedera</p>
                <p className="text-xs text-slate-600 mt-1">
                ...
                {result.prior_seal.explorer_url && <a href={result.prior_seal.explorer_url} target="_blank" rel="noreferrer" className="text-xs text-coral-600 hover:underline mt-1 inline-flex items-center gap-1">View on explorer <ExternalLink className="w-3 h-3" /></a>}
              </CardContent>
            </Card>
          )}
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-2">
                  {a.pages.map((p, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-start gap-3 text-xs bg-cream-100/40 rounded p-3" data-testid={`page-finding-${i}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-navy-800">{p.page_summary || '—'}</p>
                          {(p.findings || []).length > 0 && (
                            <ul className="mt-1 text-[11px] text-slate-600 list-disc pl-4">
          ...
          <Card className="bg-white border-slate-200 mb-4">
            <CardContent className="p-4 text-xs">
              ...
              <p className="font-mono break-all text-navy-800">{result.document_hash}</p>
              <p className="text-slate-500 uppercase tracking-wider text-[10px] font-bold mt-3 mb-1">Scan ID</p>
              <p className="font-mono text-navy-800">{result.scan_id}</p>
            </CardContent>
          </Card>

          <div className="flex gap-2 flex-wrap">
            {!result.sealed && !result.prior_seal?.found && (
              <Button onClick={() => sealScan(result.scan_id)} className="bg-coral-500 hover:bg-coral-500" data-testid="seal-scan-btn">
                <ShieldCheck className="w-4 h-4 mr-1" /> Anchor on Hedera
              </Button>
            )}
            {result.sealed && (
              <span className="inline-flex items-center gap-2 px-3 py-2 bg-coral-500/10 border border-coral-200 rounded text-xs text-coral-700" data-testid="scan-sealed">
                <CheckCircle className="w-4 h-4" /> Sealed · Seq #{result.seal?.sequence_number}
              </span>
            )}
            <Button onClick={reset} variant="outline" className="bg-cream-200 border-slate-300 text-navy-900 hover:bg-cream-200" data-testid="new-scan-btn">New scan</Button>
          </div>
        </div>
      </Shell>
    );
    ...
    <Shell>
      <div className="max-w-3xl mx-auto" data-testid="field-scanner-page">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-coral-600 text-[10px] uppercase tracking-[0.25em] font-bold">Field Document Scanner</span>
        </div>
        <h1 className="text-3xl font-bold">Scan a document</h1>
        <p className="text-slate-600 text-sm mt-1 mb-6">Capture each page, anchor on Hedera, and get AI tampering analysis.</p>

        <Card className="bg-white border-slate-200 mb-4">
          <CardContent className="p-5 space-y-3">
            <div>
              <label className="text-[10px] uppercase tracking-wider text-slate-500 font-bold block mb-1">Document label *</label>
              <Input value={label} onChange={e => setLabel(e.target.value)} placeholder="e.g. Warranty deed - 123 Oak St" className="bg-cream-100/60 border-slate-200" data-testid="scan-label" />
            </div>
            ...
            {cameraOn ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button onClick={capturePage} className="flex-1 bg-coral-500 hover:bg-coral-500" data-testid="capture-page-btn">
                    <Camera className="w-4 h-4 mr-1" /> Capture page ({pages.length})
                  </Button>
                  <Button onClick={stopCamera} variant="outline" className="bg-cream-200 border-slate-300 text-navy-900 hover:bg-cream-200" data-testid="stop-camera-btn">Done</Button>
                </div>
              </div>
            ) : (
              <div className="flex gap-2 flex-wrap">
                <Button onClick={startCamera} className="bg-coral-500 hover:bg-coral-500" data-testid="open-camera-btn">
                  <Camera className="w-4 h-4 mr-1" /> Open camera
                </Button>
                <Button onClick={() => fileInputRef.current?.click()} variant="outline" className="bg-cream-200 border-slate-300 text-navy-900 hover:bg-cream-200" data-testid="upload-page-btn">
                ...
                <Button onClick={captureGeo} variant="outline" className="bg-cream-200 border-slate-300 text-navy-900 hover:bg-cream-200" data-testid="capture-geo-btn">
                  <MapPin className="w-4 h-4 mr-1" /> {geo ? `${geo.lat.toFixed(3)},${geo.lng.toFixed(3)}` : 'Add location'}
                </Button>
                <label className="inline-flex items-center gap-2 text-xs text-slate-600 cursor-pointer ml-auto">
        ...
        {/* Page list */}
        {pages.length > 0 && (
          <Card className="bg-white border-slate-200 mb-4" data-testid="pages-list">
            <CardContent className="p-4">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                {pages.map((p, i) => (
                  <div key={i} className="relative group" data-testid={`page-thumb-${i}`}>
                    ...
                    <span className="absolute bottom-1 left-1 text-[10px] bg-black/70 text-navy-900 px-1.5 py-0.5 rounded font-mono">#{i + 1}</span>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <Button onClick={submitScan} disabled={submitting || pages.length === 0 || !label} className="w-full h-12 bg-coral-500 hover:bg-coral-500" data-testid="submit-scan-btn">
          {submitting ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Analyzing…</> : <><Sparkles className="w-4 h-4 mr-2" /> Run scan + AI check</>}
        </Button>

        {/* Recent scans */}
        {scans.length > 0 && (
          <Card className="bg-white border-slate-200 mt-8" data-testid="recent-scans">
            <CardContent className="p-0">
              ...
              {scans.map(s => {
                ...
                return (
                  <button key={s.scan_id} onClick={() => setResult(s)} className="w-full px-5 py-3 border-b border-slate-200/60 hover:bg-cream-200 grid grid-cols-12 gap-2 text-xs items-center text-left" data-testid={`scan-row-${s.scan_id}`}>
                    <FileImage className="w-4 h-4 text-coral-600 col-span-1" />
                    <span className="col-span-4 text-navy-900 font-medium truncate">{s.document_label}</span>
