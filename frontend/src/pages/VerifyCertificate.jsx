            ...
            <div data-testid="verify-cert-result">
              {result.verified ? (
                <div className="space-y-5">
                  ...
                  {/* ACN Cross-Border Passport */}
                  {result.acn_passport && result.acn_passport.public_verify_url && (
                    <Card className="bg-gradient-to-br from-coral-50 to-cream-100 border-coral-200" data-testid="cert-acn-passport">
                      <CardContent className="p-6">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <h3 className="text-navy-900 font-semibold text-lg flex items-center gap-2">
                              <Globe2 className="w-5 h-5 text-coral-600" /> Cross-Border Passport
                            </h3>
                            <p className="text-sm text-slate-600 mt-1">
                              This notarization is recognized across {(result.acn_passport.jurisdictions || []).length || '1'} jurisdiction
                              {(result.acn_passport.jurisdictions || []).length === 1 ? '' : 's'} via the
                              Autonomous Cross-Border Notarization Network.
                            </p>
                          </div>
                          {result.acn_passport.all_sealed ? (
                            <span className="px-2 py-1 rounded-md bg-emerald-100 border border-emerald-200 text-emerald-700 text-[11px] font-bold tracking-wide uppercase whitespace-nowrap">
                              All Sealed
                            </span>
                          ) : (
                            <span className="px-2 py-1 rounded-md bg-amber-100 border border-amber-200 text-amber-700 text-[11px] font-bold tracking-wide uppercase whitespace-nowrap">
                              Sealing
                            </span>
                          )}
                        </div>

                        {(result.acn_passport.jurisdictions || []).length > 0 && (
                          <div className="mt-4 flex flex-wrap gap-2" data-testid="cert-acn-juris-list">
                            {result.acn_passport.jurisdictions.map((j) => (
                              <span key={j} className="px-2.5 py-1 rounded-full bg-white border border-slate-200 text-navy-800 text-xs font-mono">
                                {j}
                              </span>
                            ))}
                          </div>
                        ...
                        <a
                          href={result.acn_passport.public_verify_url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 mt-5 px-4 py-2 rounded-lg bg-coral-500 hover:bg-coral-600 text-white text-sm transition-colors"
                          data-testid="cert-acn-verify-link"
                        >
                          <ExternalLink className="w-4 h-4" /> View cross-border passport
                        </a>
                      </CardContent>
                    </Card>
                  )}
