...
export default function AdminFLCompliance() {
  ...
  if (!isAuthenticated || user?.role !== 'admin') {
    return <Shell><Card className="bg-white border-slate-200 max-w-md mx-auto" data-testid="admin-required"><CardContent className="p-8 text-center"><Shield className="w-10 h-10 text-slate-500 mx-auto mb-2" /><p>Admin only</p></CardContent></Card></Shell>;
  }
  ...
  return (
    <Shell>
      <div className="max-w-7xl mx-auto" data-testid="admin-fl-compliance-page">
        <div className="flex items-start justify-between gap-3 mb-6 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-coral-600 text-[10px] uppercase tracking-[0.25em] font-bold">Florida · Admin Compliance</span>
            </div>
            <h1 className="text-3xl font-bold">FL compliance dashboard</h1>
            <p className="text-slate-600 text-sm mt-1">Real-time view of FL Stat. 117 obligations across all ceremonies.</p>
          </div>
          <div className="flex gap-2">
            <Button onClick={load} variant="outline" className="bg-white border-slate-300 text-navy-900 hover:bg-cream-200" data-testid="refresh-overview-btn">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Refresh'}
            </Button>
            <Link to="/admin/fl-recruitment"><Button variant="outline" className="bg-white border-slate-300 text-navy-900 hover:bg-cream-200" data-testid="goto-recruitment-btn"><Users className="w-4 h-4 mr-1" />Recruitment</Button></Link>
            <Link to="/admin/fl-ronsp"><Button variant="outline" className="bg-white border-slate-300 text-navy-900 hover:bg-cream-200" data-testid="goto-ronsp-btn"><Shield className="w-4 h-4 mr-1" />RONSP</Button></Link>
            <Link to="/admin/subpoena"><Button className="bg-coral-500 hover:bg-coral-500" data-testid="goto-subpoena-btn"><Gavel className="w-4 h-4 mr-1" />Subpoenas {overview?.subpoenas?.open ? <span className="ml-1 bg-white/20 px-1 rounded">{overview.subpoenas.open}</span> : null}</Button></Link>
          </div>
        </div>
        ...
        {/* FL ceremonies table */}
        <Card className="bg-white border-slate-200" data-testid="fl-ceremonies-table">
          <CardContent className="p-0">
            <div className="px-5 py-3 border-b border-slate-200 flex items-center justify-between">
              <h2 className="text-xs uppercase tracking-[0.2em] text-slate-500 font-bold">FL Ceremonies</h2>
              <span className="text-xs text-slate-600">{ceremonies.length} shown</span>
            </div>
            ...
            {ceremonies.map(c => (
              <div key={c.ceremony_id} className="px-5 py-3 border-b border-slate-200/60 hover:bg-cream-200 grid grid-cols-12 gap-2 text-xs items-center" data-testid={`ceremony-row-${c.ceremony_id}`}>
                <span className="col-span-3 font-mono text-navy-800 truncate">{c.ceremony_id}</span>
                <span className="col-span-3 text-slate-600 truncate">{c.user_email}</span>
                <span className="col-span-2 text-slate-500"><span className="px-2 py-0.5 rounded bg-coral-500/15 text-coral-700 text-[10px] uppercase tracking-wider font-bold">{(c.fl_nexus_basis || '').replace(/_/g, ' ')}</span></span>
                <span className="col-span-3 flex items-center gap-1.5">
                  ...
                  {c.gates.witnesses > 0 && <span className="text-[10px] text-coral-700 ml-1">W:{c.gates.witnesses}</span>}
                </span>
...
function GateChip({ label, passed, title }) {
  return (
    <span title={title} className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${passed ? 'bg-coral-500/20 text-coral-700' : 'bg-cream-200 text-slate-500'}`}>{label}</span>
  );
}
    ...
    <div className="min-h-screen bg-cream-100 text-navy-900">
      <div className="border-b border-slate-200 bg-cream-100">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center gap-3">
          <Link to="/admin" className="text-xs text-slate-600 hover:text-navy-900 inline-flex items-center gap-1"><ChevronLeft className="w-4 h-4" /> Admin</Link>
        </div>
      </div>
