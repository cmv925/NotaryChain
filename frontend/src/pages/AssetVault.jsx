...
export default function AssetVault() {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-cream-100 text-navy-900 flex items-center justify-center" data-testid="vault-login-required">
        <Card className="bg-white border-slate-200 max-w-md">
          <CardContent className="p-8 text-center">
            <Vault className="w-10 h-10 text-slate-500 mx-auto mb-2" />
            <h2 className="text-xl font-bold mb-1">Sign in required</h2>
            <p className="text-sm text-slate-600 mb-4">Your asset vault is private.</p>
  ...
  if (loading || !data) {
    return (
      <div className="min-h-screen bg-cream-100 text-navy-900 flex items-center justify-center" data-testid="vault-loading">
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-cream-100 text-navy-900 p-8" data-testid="vault-error">
  ...
  return (
    <div className="min-h-screen bg-cream-100 text-navy-900" data-testid="asset-vault-page">
      {/* Hero */}
          ...
          <div className="flex items-end justify-between gap-4 flex-wrap">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Vault className="w-5 h-5 text-coral-600" />
                <span className="text-coral-600 text-[10px] uppercase tracking-[0.25em] font-bold">Smart Asset Vault</span>
              </div>
              <h1 className="text-3xl sm:text-4xl font-bold" data-testid="vault-name">{vault.name}</h1>
              <p className="text-slate-600 text-sm mt-1">A digital safe-deposit box for your highest-value assets.</p>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setEditingSettings(true)} className="bg-white border-slate-300 text-navy-900 hover:bg-cream-200" data-testid="open-settings-btn">
                <Edit3 className="w-3.5 h-3.5 mr-1.5" /> Settings
              </Button>
              <Button onClick={() => setShowAddAsset(true)} className="bg-coral-500 hover:bg-coral-500" data-testid="add-asset-btn">
            ...
            <CardContent className="p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 min-w-0">
                  {dms.status === 'ok' ? <CheckCircle className="w-6 h-6 text-coral-600 flex-shrink-0 mt-0.5" /> :
                   dms.status === 'warning' ? <ShieldAlert className="w-6 h-6 text-coral-600 flex-shrink-0 mt-0.5" /> :
                   <AlertTriangle className="w-6 h-6 text-red-400 flex-shrink-0 mt-0.5" />}
                  <div>
                    <h3 className={`font-bold mb-0.5 ${dms.status === 'ok' ? 'text-coral-700' : dms.status === 'warning' ? 'text-coral-700' : 'text-red-300'}`}>
                ...
                <Button onClick={checkIn} size="sm" className="bg-coral-500 hover:bg-coral-500 flex-shrink-0" data-testid="check-in-btn">
            ...
            <CardContent className="p-5">
              ...
              {assets.length === 0 ? (
                <div className="text-center py-10" data-testid="assets-empty">
                  ...
                  <p className="text-sm text-slate-600 mb-4">No assets yet — add your first deed, will, or title.</p>
                  <Button onClick={() => setShowAddAsset(true)} className="bg-coral-500 hover:bg-coral-500"><Plus className="w-4 h-4 mr-1" /> Add asset</Button>
                </div>
              ) : (
                  ...
                  {assets.map(a => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left rounded-lg border p-4 transition-colors ${activeAssetId === a.asset_id ? 'bg-coral-500/10 border-emerald-500/40' : 'bg-cream-200/40 border-slate-200 hover:border-slate-300'}`}
                        data-testid={`asset-row-${a.asset_id}`}
                      >
                        <div className="flex items-start gap-3">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${isOverdue ? 'bg-red-500/15' : isDueSoon ? 'bg-coral-500/15' : 'bg-slate-700/40'}`}>
                            <Icon className={`w-5 h-5 ${isOverdue ? 'text-red-400' : isDueSoon ? 'text-coral-600' : 'text-coral-600'}`} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <h3 className="font-bold text-navy-900 truncate">{a.title}</h3>
                              <span className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-slate-700/50 text-navy-800">{ASSET_LABELS[a.asset_type] || a.asset_type}</span>
                              {a.status !== 'active' && (
                                <span className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-coral-500/15 text-coral-700">{a.status.replace(/_/g, ' ')}</span>
                              )}
                            </div>
                            <div className="flex flex-wrap items-center gap-3 mt-1 text-[11px] text-slate-500">
                              {a.value_estimate_usd && <span className="text-navy-900">${a.value_estimate_usd.toLocaleString()}</span>}
                              <span className={isOverdue ? 'text-red-400 font-bold' : isDueSoon ? 'text-coral-600 font-bold' : ''}>
                              ...
                              {a.blockchain_seal && <span className="text-coral-600">⛓ sealed</span>}
                            </div>
                          </div>
          ...
          {activeAsset ? (
            ...
            <Card className="bg-white border-slate-200 sticky top-4">
              <CardContent className="p-6 text-center">
                ...
                <p className="text-sm text-slate-600">Select an asset to view beneficiaries, re-verify, or trigger handoff.</p>
              </CardContent>
            </Card>
          )}
        </div>
...
function StatCard({ label, value, accent, testId }) {
  const accentClass = accent === 'amber' ? 'text-coral-600' : 'text-navy-900';
  return (
    <div className="rounded-lg bg-white border border-slate-200 px-4 py-3" data-testid={testId}>
        ...
        <CardContent className="p-5">
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="min-w-0">
              ...
              <h3 className="text-lg font-bold text-navy-900 truncate">{asset.title}</h3>
              {asset.description && <p className="text-xs text-slate-600 mt-1">{asset.description}</p>}
            </div>
            <button onClick={onClose} className="text-slate-500 hover:text-navy-900" aria-label="Close" data-testid="close-asset-detail">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="space-y-2 text-xs">
            ...
            {asset.blockchain_seal?.transaction_id && (
              <Row label="Hedera tx" value={(
                <a href={asset.blockchain_seal.explorer_url} target="_blank" rel="noreferrer" className="text-coral-600 hover:underline inline-flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
            <Button onClick={onVerify} size="sm" className="bg-coral-500 hover:bg-coral-500 text-xs h-9" data-testid="reverify-btn">
              <RefreshCw className="w-3 h-3 mr-1" /> Re-verify
            </Button>
            <Button onClick={onTriggerHandoff} size="sm" variant="outline"
              className="bg-coral-500/10 border-gold-500/30 text-coral-700 hover:bg-coral-500/20 text-xs h-9"
      ...
      <Card className="bg-white border-slate-200">
        <CardContent className="p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShowAddBenef(true)} className="text-coral-600 text-xs hover:text-coral-700 inline-flex items-center gap-1" data-testid="add-beneficiary-btn">
      ...
      <form onSubmit={submit} className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <Field label="Type">
            <select value={form.asset_type} onChange={(e) => setForm(f => ({ ...f, asset_type: e.target.value }))}
              className="bg-white border border-slate-300 rounded-md px-3 h-10 text-sm text-navy-900 w-full" data-testid="asset-type-select">
          ...
          <Field label="Re-verify every (days)">
            <Input type="number" min={30} max={3650} value={form.verification_interval_days}
              ...
              className="bg-white border-slate-300" data-testid="asset-interval-input" />
          </Field>
        </div>
        <Field label="Title *">
          <Input value={form.title} onChange={(e) => setForm(f => ({ ...f, title: e.target.value }))}
            placeholder="123 Main St Deed" className="bg-white border-slate-300" data-testid="asset-title-input" />
        </Field>
        <Field label="Description">
          <Textarea rows={2} value={form.description} onChange={(e) => setForm(f => ({ ...f, description: e.target.value }))}
            placeholder="Optional notes…" className="bg-white border-slate-300" data-testid="asset-description-input" />
        </Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label="Estimated value (USD)">
            <Input type="number" min={0} step="0.01" value={form.value_estimate_usd}
              ...
              placeholder="750000" className="bg-white border-slate-300" data-testid="asset-value-input" />
          </Field>
          <Field label="Jurisdiction">
            <Input value={form.jurisdiction} onChange={(e) => setForm(f => ({ ...f, jurisdiction: e.target.value }))}
              placeholder="CA, USA" className="bg-white border-slate-300" data-testid="asset-jurisdiction-input" />
          </Field>
        </div>
        <Field label="Document SHA256 hash (optional — auto-links to NotaryChain seal)">
          <Input value={form.document_hash} onChange={(e) => setForm(f => ({ ...f, document_hash: e.target.value }))}
            placeholder="64-char hex" className="bg-white border-slate-300 font-mono text-xs" data-testid="asset-hash-input" />
        </Field>
        <div className="flex justify-end gap-2 pt-2">
          <Button type="button" variant="outline" onClick={onClose} className="bg-cream-200 border-slate-300 text-navy-900 hover:bg-cream-200">Cancel</Button>
          <Button type="submit" disabled={submitting} className="bg-coral-500 hover:bg-coral-500" data-testid="submit-add-asset">
...
function AddBeneficiaryForm({ assetId, maxShare, authHeaders, onClose, onAdded }) {
  ...
  return (
    <form onSubmit={submit} className="mt-3 pt-3 border-t border-slate-200 space-y-2" data-testid="add-beneficiary-form">
      <Input value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))}
        placeholder="Beneficiary name" className="bg-white border-slate-300 text-xs h-8" data-testid="beneficiary-name-input" />
      <Input type="email" value={form.email} onChange={(e) => setForm(f => ({ ...f, email: e.target.value }))}
        placeholder="email@example.com" className="bg-white border-slate-300 text-xs h-8" data-testid="beneficiary-email-input" />
      <div className="grid grid-cols-2 gap-2">
        <Input value={form.relationship} onChange={(e) => setForm(f => ({ ...f, relationship: e.target.value }))}
          placeholder="Relationship" className="bg-white border-slate-300 text-xs h-8" data-testid="beneficiary-relationship-input" />
        <Input type="number" min={0} max={maxShare} step="0.01" value={form.share_percent}
          ...
          placeholder="Share %" className="bg-white border-slate-300 text-xs h-8" data-testid="beneficiary-share-input" />
      </div>
      <p className="text-[10px] text-slate-500">Max share available: {maxShare.toFixed(0)}%</p>
      <div className="flex justify-end gap-2">
        <Button type="button" size="sm" variant="outline" onClick={onClose} className="bg-cream-200 border-slate-300 text-navy-900 hover:bg-cream-200 text-xs h-8">Cancel</Button>
        <Button type="submit" size="sm" disabled={submitting} className="bg-coral-500 hover:bg-coral-500 text-xs h-8" data-testid="submit-beneficiary-btn">
    ...
    <Modal title="Vault settings" onClose={onClose} testId="settings-modal">
      <form onSubmit={save} className="space-y-3">
        <Field label="Vault name">
          <Input value={name} onChange={(e) => setName(e.target.value)} className="bg-white border-slate-300" data-testid="settings-name-input" />
        </Field>
        <Field label="Dead-man's-switch interval (days)">
          <Input type="number" min={30} max={3650} value={days} onChange={(e) => setDays(e.target.value)}
            className="bg-white border-slate-300" data-testid="settings-days-input" />
          <p className="text-[10px] text-slate-500 mt-1">If you don't check in within this many days, beneficiaries are auto-notified.</p>
        </Field>
        <div className="flex justify-end gap-2 pt-2">
          <Button type="button" variant="outline" onClick={onClose} className="bg-cream-200 border-slate-300 text-navy-900 hover:bg-cream-200">Cancel</Button>
          <Button type="submit" disabled={saving} className="bg-coral-500 hover:bg-coral-500" data-testid="settings-save-btn">
...
function Modal({ title, children, onClose, testId }) {
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose} data-testid={testId}>
      <div className="bg-white border border-slate-300 rounded-lg max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-slate-200">
          <h2 className="font-bold text-lg">{title}</h2>
          <button onClick={onClose} className="text-slate-600 hover:text-navy-900" aria-label="Close" data-testid="modal-close"><X className="w-4 h-4" /></button>
        </div>
