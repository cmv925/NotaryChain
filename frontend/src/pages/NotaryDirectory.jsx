    ...
    <div className="min-h-screen bg-cream-100 text-navy-900" data-testid="notary-directory-page">
      ...
      <div className="border-b border-slate-200 bg-cream-100">
        <div className="max-w-6xl mx-auto px-6 py-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-coral-600 text-[11px] uppercase tracking-[0.25em] font-bold">Notary Directory</span>
          </div>
          <h1 className="font-serif text-4xl sm:text-5xl font-bold tracking-tight text-navy-900 mb-2">Find a verified NotaryChain notary.</h1>
      ...
      <div className="max-w-6xl mx-auto px-6 py-6">
        <form onSubmit={submit} className="flex flex-col md:flex-row gap-3">
          ...
          <select
            ...
            className="bg-white border border-slate-200 rounded-md px-3 h-10 text-sm text-navy-900"
            data-testid="directory-state-filter"
          >
            <option value="">All states</option>
            {US_STATES.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <Button type="submit" className="bg-coral-500 hover:bg-coral-500" data-testid="directory-search-btn">
      ...
      <div className="max-w-6xl mx-auto px-6 pb-12">
        ...
        {!loading && (data.notaries || []).length === 0 && (
          <Card className="bg-white border-slate-200" data-testid="directory-empty">
        ...
        {!loading && (data.notaries || []).length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" data-testid="directory-results">
            {data.notaries.map(n => (
              ...
              >
                <Card className="bg-white border-slate-200 hover:border-emerald-500/40 transition-colors h-full">
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="min-w-0">
                        <h3 className="font-bold text-navy-900 truncate group-hover:text-coral-700 transition-colors">{n.name || '—'}</h3>
                        <p className="text-[11px] text-slate-500 font-mono truncate">{n.notary_id}</p>
                      </div>
                      <span className={`text-[9px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full whitespace-nowrap ${n.bond_active ? 'bg-coral-500/15 text-coral-700' : 'bg-slate-700/50 text-slate-600'}`}>
                    ...
                    <div className="space-y-1.5 text-xs">
                      {n.license_state && (
                        <div className="flex items-center gap-1.5 text-slate-600">
                          <MapPin className="w-3.5 h-3.5 text-coral-600/70" />
                      ...
                      {n.bond_amount_usd && (
                        <div className="flex items-center gap-1.5 text-slate-600">
                          <Award className="w-3.5 h-3.5 text-coral-600/70" />
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-200 flex items-center justify-between text-[11px]">
                      <div>
                        ...
                        <span className="ml-1.5 font-bold text-navy-900">{(n.total_seals || 0).toLocaleString()}</span>
                      </div>
                      <span className="text-coral-600 group-hover:text-coral-700 inline-flex items-center gap-1">
        ...
        {!loading && totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 mt-8" data-testid="directory-pagination">
            <Button
              ...
              className="bg-white border-slate-200 text-navy-900 hover:bg-cream-200"
      ...
      <div className="border-t border-slate-200">
        <div className="max-w-6xl mx-auto px-6 py-10 grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
          <div>
            ...
            <Link to="/notary/onboarding" className="text-coral-600 text-xs hover:underline">Onboard now →</Link>
          </div>
          <div>
            ...
            <Link to="/verify" className="text-coral-600 text-xs hover:underline">Open Verify →</Link>
          </div>
          <div>
