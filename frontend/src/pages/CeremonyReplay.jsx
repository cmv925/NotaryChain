...
function StepCard({ step, isActive, isCompleted, delay }) {
  ...
  return (
    <div
      ...
      } ${isCompleted ? statusColors[step.status] || statusColors.completed : 'border-slate-800 bg-slate-900/20 opacity-40'}`}
      style={{ animationDelay: `${delay}ms`, animation: isCompleted ? 'fadeInUp 0.6s ease forwards' : 'none' }}
    >
      <div className="flex items-start gap-3">
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
          step.status === 'pass' ? 'bg-emerald-500/15 text-emerald-400' :
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {step.verdict && (
              <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${
                step.verdict === 'PASS' ? 'bg-emerald-500/15 text-emerald-400' : 'bg-red-500/15 text-red-400'
              }`}>{step.verdict}</span>
            )}
            {step.result && (
              <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${
                step.result === 'APPROVED' ? 'bg-emerald-500/15 text-emerald-400' : 'bg-red-500/15 text-red-400'
              }`}>{step.result}</span>
            )}
          </div>
          ...
          {step.checks?.length > 0 && isCompleted && (
            <div className="flex flex-wrap gap-1 mt-2">
              {step.checks.map((c, i) => (
                <span key={i} className="text-[9px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded">{c}</span>
              ))}
            </div>
          )}
          {step.explorer_url && (
    ...
    <div className="min-h-screen bg-[#0a0f1a]" data-testid="ceremony-replay-page">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mt-4 mb-6">
          <div>
            <h1 className="text-xl font-bold text-white">{data.document_name}</h1>
        ...
        {currentStep >= data.steps.length && (
          <div data-testid="replay-complete" className="mt-6 text-center bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4">
            <CheckCircle className="w-6 h-6 text-emerald-400 mx-auto mb-2" />
            <p className="text-sm font-semibold text-emerald-400">Ceremony Complete</p>
