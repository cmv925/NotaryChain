...
export default function BrandingPage() {
  ...
  if (loading) return <div className="min-h-screen bg-cream-100 flex items-center justify-center"><Loader2 className="w-6 h-6 animate-spin text-slate-500" /></div>;
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">Personalize your organization's appearance</p>
          </div>
        </div>

        <div className="space-y-6">
          {/* Logo */}
          <Card className="bg-cream-100 border-slate-200" data-testid="logo-section">
            <CardHeader><CardTitle className="text-base text-navy-900">Logo</CardTitle></CardHeader>
            <CardContent className="flex items-center gap-6">
              <div className="w-20 h-20 rounded-xl bg-white border border-slate-200 flex items-center justify-center overflow-hidden">
                {branding?.logo_url
              ...
              <div>
                <label className="cursor-pointer">
                  <Button variant="outline" className="border-slate-200 text-slate-500" asChild>
                ...
                <p className="text-slate-600 text-xs mt-1">PNG, JPG, or SVG. Max 2MB.</p>
              </div>
            </CardContent>
          </Card>

          {/* Display Name & Tagline */}
          <Card className="bg-cream-100 border-slate-200" data-testid="identity-section">
            ...
            <CardContent className="space-y-3">
              <div>
                ...
                <Input value={branding?.display_name || ''} onChange={(e) => update('display_name', e.target.value)} placeholder="Your Company Name" className="bg-white border-slate-200 text-navy-900" data-testid="display-name" />
              </div>
              <div>
                <label className="text-slate-500 text-xs block mb-1">Tagline</label>
                <Input value={branding?.tagline || ''} onChange={(e) => update('tagline', e.target.value)} placeholder="Trusted Digital Notarization" className="bg-white border-slate-200 text-navy-900" data-testid="tagline" />
              </div>
            </CardContent>
          </Card>

          {/* Colors */}
          <Card className="bg-cream-100 border-slate-200" data-testid="colors-section">
              ...
              {[
                ...
                <div key={key} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-slate-500 text-xs">{branding?.[key] || '#00d4aa'}</p>
                  </div>
            ...
            <CardContent className="pt-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs">{branding?.tagline || 'Your tagline here'}</p>
                </div>
              </div>
