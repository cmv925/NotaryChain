      ...
      <CardHeader className="pb-2 cursor-pointer" onClick={() => toggle(id)}>
        <CardTitle className="text-sm text-navy-900 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {badge}
            {expanded[id] ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
          </div>
        </CardTitle>
      </CardHeader>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-slate-500 text-sm">Forensic-grade transaction evidence bundle</p>
          </div>
        </div>

        {/* Actions */}
        <div className="flex gap-3 mb-6">
          <Button onClick={fetchPackage} disabled={loading} variant="outline" className="border-slate-200 text-slate-500" data-testid="fetch-package-btn">
        ...
        {pkg && (
          <div className="space-y-4">
            {/* Overview */}
            <Card className="bg-gradient-to-r from-cream-100 to-emerald-100 border-coral-200" data-testid="package-overview">
              <CardContent className="pt-5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-cream-100 rounded-lg p-3 text-center">
                    <p className="text-slate-500 text-[10px]">Package ID</p>
                  ...
                  <div className="bg-cream-100 rounded-lg p-3 text-center">
                    <p className="text-slate-500 text-[10px]">Progress</p>
                    <p className="text-navy-900 text-xs">{pkg.transaction?.progress}%</p>
                  </div>
                  <div className="bg-cream-100 rounded-lg p-3 text-center">
                    <p className="text-slate-500 text-[10px]">Integrity</p>
            ...
            <Section title="Blockchain Proof" icon={Shield} color="text-coral-600" id="blockchain"
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between"><span className="text-slate-500">Network</span><span className="text-navy-900">{pkg.blockchain_proof?.network}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Topic ID</span><span className="text-navy-900 font-mono">{pkg.blockchain_proof?.topic_id || '-'}</span></div>
                {pkg.blockchain_proof?.settlement_hash && (
                  <div>
                    <span className="text-slate-500 block mb-1">Settlement Hash</span>
                    <div className="flex items-center gap-2">
                      <code className="text-coral-600 bg-white px-2 py-1 rounded text-[10px] break-all flex-1">{pkg.blockchain_proof.settlement_hash}</code>
                ...
                {pkg.participants?.map((p, i) => (
                  <div key={i} className="flex items-center justify-between bg-white rounded-lg p-2">
                    <div>
                      ...
                      <span className="text-slate-500 text-[10px] ml-2">{p.email}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-[10px] border-slate-200 text-slate-500">{p.role}</Badge>
                      <Badge className={p.status === 'joined' ? 'bg-green-500/15 text-green-400 text-[10px]' : 'bg-gray-500/15 text-slate-500 text-[10px]'}>
                        {p.status}
                      </Badge>
                    </div>
                  </div>
            ...
            <Section title={`Tasks (${pkg.tasks?.length || 0})`} icon={FileText} color="text-coral-600" id="tasks">
              <div className="space-y-1">
                {pkg.tasks?.map((t, i) => (
                  <div key={i} className="flex items-center justify-between py-1.5 border-b border-slate-200/50 last:border-0">
                    <div className="flex items-center gap-2">
                      {t.status === 'completed' ? <CheckCircle className="w-3 h-3 text-green-400" /> : <XCircle className="w-3 h-3 text-slate-600" />}
                      <span className={`text-xs ${t.status === 'completed' ? 'text-slate-500' : 'text-slate-500'}`}>{t.name}</span>
                    </div>
                ...
                {pkg.biometric_evidence?.passports?.map((bp, i) => (
                  <div key={i} className="bg-white rounded-lg p-2">
                    <div className="flex justify-between">
                      <span className="text-slate-500">Passport #{bp.id?.substring(0, 8)}</span>
                    ...
                    <p className="text-slate-600 text-[10px] mt-1">Score: {(bp.composite_score * 100).toFixed(1)}%</p>
                  </div>
                ))}
              </div>
            </Section>
            ...
            <Section title="Communication Record" icon={MessageSquare} color="text-teal-400" id="communication">
              <div className="space-y-2 text-xs">
                <div className="flex justify-between"><span className="text-slate-500">Total Messages</span><span className="text-navy-900">{pkg.communication?.total_messages || 0}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">AI Conductor Sessions</span><span className="text-navy-900">{pkg.communication?.conductor_guidance_sessions || 0}</span></div>
              </div>
            </Section>

            {/* Component Hashes */}
            {pkg.component_hashes && (
              <Section title="Component Hashes" icon={Shield} color="text-slate-500" id="hashes">
                <div className="space-y-2">
        ...
        {!pkg && !loading && !generating && (
          <Card className="bg-cream-100 border-slate-200">
            <CardContent className="py-16 text-center">
              <Package className="w-12 h-12 mx-auto mb-3 text-slate-600 opacity-30" />
              <p className="text-slate-500 text-sm">No evidence package generated yet</p>
              <p className="text-slate-600 text-xs mt-1">Click "Generate New Package" to compile all transaction evidence</p>
            </CardContent>
          </Card>
        )}
      </div>
