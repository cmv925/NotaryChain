      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">Multi-modal biometric identity credentials</p>
          </div>
        </div>

        {/* Generate Section */}
        <Card className="bg-cream-100 border-slate-200 mb-6">
          <CardContent className="pt-5">
            <h3 className="text-navy-900 text-sm font-semibold mb-3">Generate Passport from Session</h3>
            <p className="text-slate-500 text-xs mb-3">Enter the session ID from your biometric verification to generate a unified Biometric Passport.</p>
            ...
            <CardContent className="pt-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs">Hash: {verifyResult.biometric_hash?.substring(0, 24)}...</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div><span className="text-slate-500">Score:</span> <span className="text-navy-900">{(verifyResult.composite_score * 100).toFixed(1)}%</span></div>
                <div><span className="text-slate-500">Modalities:</span> <span className="text-navy-900">{verifyResult.modalities_verified?.join(', ')}</span></div>
                <div><span className="text-slate-500">Issued:</span> <span className="text-navy-900">{verifyResult.issued_at ? new Date(verifyResult.issued_at).toLocaleDateString() : '-'}</span></div>
                <div><span className="text-slate-500">Expires:</span> <span className="text-navy-900">{verifyResult.expires_at ? new Date(verifyResult.expires_at).toLocaleDateString() : '-'}</span></div>
              </div>
            </CardContent>
          </Card>
        )}
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {passports.map((p) => (
              <Card key={p.id} className="bg-cream-100 border-slate-200" data-testid={`passport-${p.id}`}>
                <CardContent className="pt-5">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-slate-500 text-xs">Session: {p.session_id?.substring(0, 16)}...</p>
                      </div>
                    </div>
                  ...
                  <div className="flex gap-2 mt-3">
                    {p.modalities_verified?.map((m) => (
                      <Badge key={m} variant="outline" className="text-[10px] border-slate-200 text-slate-500">
                        {modalityLabels[m] || m}
                      </Badge>
                    ))}
                  </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3">
                      {Object.entries(p.modality_details).map(([key, val]) => (
                        <div key={key} className="bg-white rounded p-2 text-center">
                          <p className="text-slate-500 text-[10px]">{modalityLabels[key] || key}</p>
